...
class RockMigrations extends WireData implements Module, ConfigurableModule
{
  ...
  private function createConstantTraits(): void
  {
    ...
    if (count($const)) {
      ...
      if (is_file($dst)) {
        ...
        foreach ($const as $file) $content .= $this->getConfigFileConstant($file);
        $content .= "}\n";
    ...
    foreach ($constants as $tag => $files) {
      ...
      foreach ($files as $file) $content .= $this->getConfigFileConstant($file);
      $content .= "}\n";
  ...
  private function getConfigFileConstant(string $file): string
  {
    $type = $this->getConfigFileType($file, true);
    $shortName = $this->getConfigFileName($file, true, true);
    if (str_starts_with($shortName, '.')) return '';
